...
def convert_to_rgb(image_bytes):
    """Convert any image to RGB mode for JPEG compatibility"""
    try:
...
async def register_passenger(
    ...
    try:
        ...
        rgb_contents = convert_to_rgb(contents)
        
        # Try to encode face
        encoding = encode_face(rgb_contents)
...
async def auto_detect_face(file: UploadFile = File(...), camera_id: str = Form("")):
    ...
    try:
        contents = await file.read()
        rgb_contents = convert_to_rgb(contents)
        matched_person, confidence = match_face_in_db(rgb_contents)
